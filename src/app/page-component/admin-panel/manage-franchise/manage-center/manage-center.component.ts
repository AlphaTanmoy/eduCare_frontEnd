...
import { faEdit, faEye, faDownload, faCircleInfo, faCircleXmark, faLockOpen } from '@fortawesome/free-solid-svg-icons';
  ...
  DownloadFranchiseDocuments(center_id: string) {
    ...
    this.franchiseService.GetFileStreamByFolderAndFilename(center_id, FranchiseDocumentName.SUPPORTABLE_DOCUMENT).subscribe({
      next: async (blob: Blob) => {
        ...
        link.download = `${center_id}_${FranchiseDocumentName.SUPPORTABLE_DOCUMENT}`;
        link.click();
      },
      error: (err) => {
        this.hideMatProgressBar();
