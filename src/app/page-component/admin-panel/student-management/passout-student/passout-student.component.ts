...
export class PassoutStudentComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  async getStudents(page: number, size: number) {
    try {
      ...
      const res = await firstValueFrom(this.studentService.getAllAvailablePassoutStudents(page, size));
      if (res.status !== 200) {
